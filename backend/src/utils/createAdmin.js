...
async function createAdminAccount() {
  ...
  try {
    ...
    if (managerExists) {
      ...
      await managerExists.update({ password: hashedManagerPw, role: 'manager', status: 'active' });
      console.log(`√ Cập nhật tài khoản manager test: ${managerEmail}`);
    } else {
      const hashedManagerPw = await bcrypt.hash(managerPassword, 10);
      await User.create({ email: managerEmail, password: hashedManagerPw, role: 'manager', status: 'active' });
