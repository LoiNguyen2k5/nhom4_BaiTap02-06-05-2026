...
async function seedData() {
  try {
    ...
    const kpiData = [
      { user_id: EMP1, reviewer_id: MGR1, month: now.getMonth() || 12, year: now.getMonth() ? now.getFullYear() : now.getFullYear()-1, kpi_score: 85, rating: 'B', comments: 'Hoàn thành 85% KPI. Tốt ở phần backend, cần cải thiện tốc độ fix bug frontend. Tiếp tục phát huy tinh thần học hỏi.' },
      { user_id: EMP2, reviewer_id: MGR1, month: now.getMonth() || 12, year: now.getMonth() ? now.getFullYear() : now.getFullYear()-1, kpi_score: 96, rating: 'A', comments: 'Xuất sắc! Vượt KPI 20%. Thiết kế kiến trúc hệ thống rất chuyên nghiệp. Là tấm gương cho cả team. Xem xét thăng chức lên Senior.' },
      { user_id: EMP3, reviewer_id: MGR1, month: now.getMonth() || 12, year: now.getMonth() ? now.getFullYear() : now.getFullYear()-1, kpi_score: 70, rating: 'C', comments: 'Cần cải thiện. Một số task bị trễ deadline. Kỹ năng kỹ thuật còn hạn chế, cần tham gia khóa đào tạo thêm trong Q3.' },
      { user_id: EMP4, reviewer_id: MGR1, month: now.getMonth() || 12, year: now.getMonth() ? now.getFullYear() : now.getFullYear()-1, kpi_score: 88, rating: 'B', comments: 'Làm việc tốt, nhiệt tình hỗ trợ team members. Nên chủ động hơn trong việc đề xuất cải tiến quy trình.' },
      { user_id: EMP5, reviewer_id: MGR1, month: now.getMonth() || 12, year: now.getMonth() ? now.getFullYear() : now.getFullYear()-1, kpi_score: 92, rating: 'A', comments: 'Nghiên cứu AI/ML rất tốt. Đề xuất giải pháp sáng tạo và khả thi. Đặc biệt ấn tượng với POC nhận diện khuôn mặt.' },
      { user_id: EMP6, reviewer_id: MGR2, month: now.getMonth() || 12, year: now.getMonth() ? now.getFullYear() : now.getFullYear()-1, kpi_score: 80, rating: 'B', comments: 'Chiến dịch marketing Q2 đạt kết quả tốt. ROI vượt kỳ vọng 15%. Cần cải thiện kỹ năng báo cáo số liệu.' },
      { user_id: EMP7, reviewer_id: MGR2, month: now.getMonth() || 12, year: now.getMonth() ? now.getFullYear() : now.getFullYear()-1, kpi_score: 73, rating: 'C', comments: 'Mới tham gia team, đang trong giai đoạn học việc. Cần thêm 1 tháng để đánh giá chính xác hơn.' },
      { user_id: EMP9, reviewer_id: MGR1, month: now.getMonth() || 12, year: now.getMonth() ? now.getFullYear() : now.getFullYear()-1, kpi_score: 91, rating: 'A', comments: 'Kinh doanh xuất sắc! Đạt 135% chỉ tiêu doanh số Q2. Ký được 3 hợp đồng lớn với khách hàng enterprise.' },
    ];
